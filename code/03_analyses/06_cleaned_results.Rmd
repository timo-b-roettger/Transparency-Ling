...
```{r data_imports}
DF <- import("../../data/03_analyses/data_typofixes.csv")
...
# update this table based on 04_data_coding
reasons_excluded[1,1] <- reasons_excluded[1,1] + reasons_excluded[4,1]
reasons_excluded[4,1] <- 0

# total exclude 
total_excluded <- sum(reasons_excluded[ , 1]) + sum(reasons_excluded[1:3, 2])
```

Sample characteristics for all `r sample_size` articles are displayed in Table 2 and Figure 1. `r total_excluded` (`r round(total_excluded/sample_size*100, 1)`%) Articles were excluded due to not being in English (`r round(sum(reasons_excluded[2, ])/sample_size*100, 1)`%), not being about language according to our definition above (`r round(sum(reasons_excluded[1, ])/sample_size*100, 1)`%), or for not being accessible (`r round(sum(reasons_excluded[3, ])/sample_size*100, 1)`%).
...
levels(fig2_data2$year_split) <- c("Pre-RC (2008/2009)", "Post-RC (2018/2019)")
