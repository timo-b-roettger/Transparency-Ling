...
```{r}
DF_Second <- DF %>% filter(!is.na(round2_coder)) %>% 
...
DF_Second_import <- DF_Second_import %>% 
  mutate(answer = as.character(answer)) %>% 
  bind_rows(import("missed_these_coded.xlsx"))
...
DF_Second_match <- DF %>% filter(!is.na(round2_coder)) %>% 
  ...
  pivot_wider(id_cols = c(ArticleID, name), 
              ...
              values_from = value) %>% 
  mutate(easy_screen = coder_1 == coder_2) %>% 
  left_join((DF_Second_import %>% select(ArticleID, answer, name)), 
...
DF_Second_match$easy_screen[is.na(DF_Second_match$coder_1) & is.na(DF_Second_match$coder_2)] <- TRUE

table("easy" = DF_Second_match$easy_screen, 
      "coded" = DF_Second_match$answer, 
      useNA = "ifany")

# I forgot to include the NA mismatches 
# export(DF_Second_match %>% filter(is.na(easy_screen)), "missed_these.csv", row.names = F)

# copy it over 
DF_Second_match$final_answer <- DF_Second_match$easy_screen
table(DF_Second_match$final_answer, useNA = "ifany")

# fix NAs in the easy_screen with answer
DF_Second_match$final_answer[is.na(DF_Second_match$final_answer)] <- DF_Second_match$answer[is.na(DF_Second_match$final_answer)]
table(DF_Second_match$final_answer, useNA = "ifany")

# fix answers that don't agree
DF_Second_match$final_answer[(DF_Second_match$easy_screen != DF_Second_match$answer)
                             & !is.na(DF_Second_match$answer) & !is.na(DF_Second_match$easy_screen)] <- DF_Second_match$answer[(DF_Second_match$easy_screen != DF_Second_match$answer)
                             & !is.na(DF_Second_match$answer) & !is.na(DF_Second_match$easy_screen)]
table(DF_Second_match$final_answer, useNA = "ifany")

table(DF_Second_match$final_answer, 
