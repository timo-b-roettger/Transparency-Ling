...
#####print out the articles that do the thing#####
...
# how many different journals in each split
...
```{r}
DF$OriginalArticleLanguage <- DF$ArticleLanguage

# output of original text
table(DF$OriginalArticleLanguage)
...
describeBy(count_lang$sample_size, count_lang$year_split)
...
bar_languages <- 
  ...
  labs(title = "B: Proportion of languages in sample",
       x = "",
       y = "") + 
...
country_summary$Region <- countrycode(
  sourcevar = country_summary$CountryCode, 
  origin = 'iso2c', 
  destination = "un.regionsub.name")
country_summary$Region[country_summary$CountryCode == "TW"] <- "Eastern Asia"
...
# Put JIF and country plot together
sample_characteristics <- JIF_plot  + bar_languages + plot_layout(widths = c(1, 0.7))
...
country_summary2 <- DF %>% 
      group_by(CountryCode, year_split) %>% 
      summarize(n = n()) 

country_summary2$Region <- countrycode(
  sourcevar = country_summary2$CountryCode, 
  origin = 'iso2c', 
  destination = "un.regionsub.name")
country_summary2$Region[country_summary2$CountryCode == "TW"] <- "Eastern Asia"

country_summary2$year_split <- factor(
  country_summary2$year_split,
  levels = c("Pre-OS", "After-OS"))

library(treemapify)
ggplot(country_summary2 %>% filter(!is.na(Region)), 
       aes(area = n, fill = CountryCode,
...
studyDF$study <- sub("description of archive of stuttered speech", "corpus study", studyDF$study)
studyDF$study <- gsub("correlational study|formal phonology|introspection|just selected examples|case study|meta analysis|modelling|^na$|secondary data from previous reports|simulation|test battery on language   cognitive skills", "other", studyDF$study)
...
table(studyDF$study, useNA = "ifany")

table(studyDF$study, studyDF$year_split, useNA = "ifany")
...
```{r}
# first river plot attempts
# remotes::install_github("corybrunson/ggalluvial", ref = "optimization")
library(ggalluvial)
library(ggfittext)

# make fake data, wrangle real data later
dummy_df <- data.frame(study_type = sample(c("primary", "secondary", "other"), 450, replace = TRUE),
                       data_type = c(rep("text", 300), rep("audio", 100), rep("video", 50)),
                       available = c(rep("yes", 50), rep("no", 250),
                                     rep("yes", 20), rep("no", 80),
                                     rep("yes", 10), rep("no", 40))) %>% 
  mutate(study_type = factor(study_type, levels = c("primary", "secondary", "other"))) %>% 
  group_by(study_type, data_type, available) %>% 
  summarize(sum = n())

ggplot(data = dummy_df,
       aes(axis1 = study_type, 
           axis2 = data_type, 
           axis3 = available,
           fill = available,
           y = sum)) +
  scale_x_discrete(limits = c("study type", "data type", "available"), expand = c(.2, .05)) +
  scale_fill_manual(values = c("#b8627dff", "#22A884FF")) + 
  labs(title = "data availability across study and data types",
       y = "",
       x = "") +
  geom_alluvium() +
  geom_stratum(color = c("grey", "grey", "grey",
                        "grey", "grey", "grey",
                        "#22A884FF", "#b8627dff"), 
