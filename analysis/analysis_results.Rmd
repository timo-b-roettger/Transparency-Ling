...
# as per github page 
prescreenDF$ID[prescreenDF$Title == "Quantified epistemic logics for reasoning about knowledge in multi-agent systems"] <- "eac0c983.1"

# import the OA code
openaccessDF <- import("OA_Coding_10_2021.xlsx")
```
...
# take out duplicates by same person
DF <- DF %>% 
  filter(!duplicated(ArticleID, Coder))

nrow(DF)
table(DF$Coder)

# find missing ID
#### do we have a version of the github issue I can import to find the one missing ? ####

# separate out the second screener of the 20%
DF$SecondCoder <- duplicated(DF$ArticleID)

table(DF$SecondCoder)
```
...
# deal with NO ACCESS versus NA, maybe new column
DF$JIF <- gsub("NA", NA, DF$JIF)

jifDF <- DF %>% 
  select(JIF, JIFYear, year_split, Year) %>% 
  mutate(JIF_no = JIF == "NO ACCESS") 
...
```{r}
table(DF$StudyType)
table(DF$EmpiricalStudyData)

# maybe top ten options + other 

library(ggplot2)

ggplot(DF, aes(EmpiricalStudyData)) + 
  geom_bar() +
  theme_classic()
```
