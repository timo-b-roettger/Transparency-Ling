...
bar_countries <- 
  ggplot(country_summary, aes(x = year_split, 
                         y =  prop, 
                         fill = CountryCode2)) +
  geom_bar(stat = "identity",
           #col = "black",
...
table1 <- data.frame(
  ...
  "Total" = c(
    ...
    sum(grepl("^Primary", DF$EmpiricalStudyData)), 
    sum(grepl("^Second", DF$EmpiricalStudyData)), 
    sum(grepl("^Other", DF$EmpiricalStudyData))
    ),
  ## pre stats 
  "Pre-OS" = c(
    ...
    sum(grepl("^Primary", DF$EmpiricalStudyData[DF$year_split == "Pre-OS"])), 
    sum(grepl("^Second", DF$EmpiricalStudyData[DF$year_split == "Pre-OS"])), 
    sum(grepl("^Other", DF$EmpiricalStudyData[DF$year_split == "Pre-OS"]))
    ),
  ...
  "After-OS" = c(
     ...
     sum(grepl("^Primary", DF$EmpiricalStudyData[DF$year_split == "After-OS"])), 
     sum(grepl("^Second", DF$EmpiricalStudyData[DF$year_split == "After-OS"])), 
     sum(grepl("^Other", DF$EmpiricalStudyData[DF$year_split == "After-OS"]))
     )
)
...
# figure 1 
bar_countries  + bar_languages + plot_layout(widths = c(1.25, 1.25))
```
...
DF_filtered_ps$RawDataAvl <- sub(".*corp.*|^Corpus.*", "Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub("No statement but links to data sources|Raw data \\(examples\\) are provided within the publication", "Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub(".*third.*", "Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub("State that the data is no longer available", "Not Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub("Some raw data available", "Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub("No, raw data.*", "Not Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl <- sub("^Yes.*", "Avaliable", DF_filtered_ps$RawDataAvl)
DF_filtered_ps$RawDataAvl[!is.na(DF_filtered_ps$EmpiricalStudyData) & is.na(DF_filtered_ps$RawDataAvl)] <- "Not Avaliable"
...
rawdata_access <- rawdata_graph %>% filter(RawDataAvl == "Avaliable")
...
DF_filtered_ps$ProcessData <- sub("^No.*|^Points.*", "Not Avaliable", DF_filtered_ps$ProcessData)
DF_filtered_ps$ProcessData <- sub("^Yes.*|.*raw data.*", "Avaliable", DF_filtered_ps$ProcessData)
DF_filtered_ps$ProcessData[is.na(DF_filtered_ps$ProcessData) & !is.na(DF_filtered_ps$EmpiricalStudyData)] <- "Not Avaliable"
...
processdata_access <- processdata_graph %>% filter(ProcessData == "Avaliable")
...
For the `r sum(processdata_access$n)`  articles for which derived data were reportedly available, the data were not actually available for `r nrow(DF_filtered_ps %>% filter(ProcessDataWhere == "Dead"))` articles because of broken links or availability "upon request". For the articles that we could access, the derived data were made available in a journal-hosted supplement (n = `r nrow(DF_filtered_ps %>% filter(ProcessData == "Avaliable" & ProcessDataWhere == "Supplemental"))`), on a personal or institutionally hosted (nonrepository) webpage (n = `r nrow(DF_filtered_ps %>% filter(ProcessDataWhere == "Personal"))`), or in an online third-party repository (n = `r nrow(DF_filtered_ps %>% filter(ProcessDataWhere == "Third-Party"))`). Of those articles, `r nrow(DF_filtered_ps %>% filter(ProcessDataDocument == "Yes"))` data sources were documented.

## 3.5 Analysis-script availability

```{r code-analysis}
DF_filtered_ps$AnalysisScript[DF_filtered_ps$AnalysisScript == ""] <- NA
DF_filtered_ps$AnalysisScript <- sub("^No.*|^Analysis.*|^Link.*", "Not Avaliable", DF_filtered_ps$AnalysisScript)
DF_filtered_ps$AnalysisScript <- sub("^Yes.*", "Avaliable", DF_filtered_ps$AnalysisScript)
DF_filtered_ps$AnalysisScript[!is.na(DF_filtered_ps$EmpiricalStudyData) & is.na(DF_filtered_ps$AnalysisScript)] <- "Not Avaliable"
...
```{r figure2, echo = FALSE, include = TRUE, fig.cap = "Figure 2. Pre-OS (left) and After-OS (right) percentages of the availability of materials, raw data, processed data, and analysis scripts."}
# combine everything
fig2_data <- bind_rows(
  materials_graph %>% 
...
ggplot(fig2_data, aes(EmpiricalStudyData, prop, fill = Avaliable)) + 
...
levels(fig2_data2$year_split) <- c("Pre-OS (2008-09)", "After-OS (2018-2019)")

fig2_data_label <- fig2_data2 %>%
  mutate(label = paste0(prop, "% ", "(n = ", n, ")"),
         label = ifelse(Avaliable != " Avaliable", label, ""),
         label2 = ifelse(prop > 10 & Avaliable == " Avaliable", paste0(prop, "%"), "")
       )
